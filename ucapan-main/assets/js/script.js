...
hilang1.addEventListener('click', () => {
  ...
  Swal.fire({
    ...
    imageHeight: 120,
    title: 'Halooo cewekku yang paling cantikk, imutt, lucu, maniss dan menggemaskannn :)',
    html: `<input type="text" id="login" class="swal2-input" placeholder="isi sembarang ajaa">`,
    confirmButtonText: 'Kirim',
    focusConfirm: false,
    preConfirm: () => {
      const login = Swal.getPopup().querySelector('#login').value
      if (!login) {
        Swal.showValidationMessage(`kamu`)
      }
      return { login }
    }
  }).then((result) => {
    ...
    let txt = `sayang, aku mau minta maaf ke kamu, karena aku udah bikin kamu kecewa, kesal, dan badmood. ${pesan}`;
